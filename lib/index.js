    ...
    switch ( element.type || "object" )
    {
        case "string":
        {
            if ( _.isArray( element.enum ) && !_.isEmpty( element.enum ) && options.noStringLiterals !== true )
            {
                ...
                type     = "type " + typeName + " = \"" + element.enum.join( "\" | \"" ) + "\";";
        ...
        case "array":
        {
            ...
            if ( _.isObject( element.items ) )
            {
                ...
                {
                    var elementPrefix = element.items.type === "string" ? options.typePrefix : options.prefix;
                    output = name + ( required ? "" : "?" ) + ": Array<" + elementPrefix + options.nameMapping( element.items.$ref, options.pathDepth ) + ">;";
                }
                else
                {
        ...
        case "object":
        {
            ...
            if ( element.$ref )
            {
                var elementPrefix = element.type === "string" ? options.typePrefix : options.prefix;
