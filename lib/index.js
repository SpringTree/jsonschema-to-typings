    ...
    switch ( element.type || "object" )
    {
        case "string":
        {
            if ( _.isArray( element.enum ) && !_.isEmpty( element.enum ) && options.noStringLiterals !== true )
            {
                ...
                //
                type   = "type " + options.typePrefix + name + " = \"" + element.enum.join( "\" | \"" ) + "\";";
        ...
        case "array":
        {
            ...
            if ( _.isObject( element.items ) )
            {
                ...
                {
                    output = name + ( required ? "" : "?" ) + ": Array<" + options.prefix + options.nameMapping( element.items.$ref, options.pathDepth ) + ">;";
                }
                else
                {
        ...
        case "object":
        {
            ...
            if ( element.$ref )
            {
                output = name + ( required ? "" : "?" ) + ": " + options.prefix + options.nameMapping( element.$ref, options.pathDepth ) + ";";
            }
    ...
    if ( schema.id )
    {
        ...
        if ( _.isArray( schema.allOf ) )
        {
            _.forEach( schema.allOf, function( allOff )
            {
                if ( allOff.$ref )
                {
                    extend.push( options.nameMapping( allOff.$ref, options.pathDepth ) );
                }
            } );
        }
        ...
        if ( extend.length )
        {
            output.push(  "  interface " + name + " extends " + extend.join( ", " ) + " {"  );
        }
        else
        {
...
module.exports = function( schemas, options )
{
    ...
    _.forEach( schemas, function( schema )
    {
        ...
        else if ( schema.type === "string" && _.isArray( schema.enum ) && !_.isEmpty( schema.enum ) && options.noStringLiterals !== true )
        {
            ...
            if ( schema.id )
            {
                var name    = options.nameMapping( schema.id, options.pathDepth );
